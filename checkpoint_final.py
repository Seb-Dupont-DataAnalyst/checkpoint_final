...
if choice == 'Analyse du marché':
    ...
    fig.update_layout(
    annotations=[dict(text=df['price'].median(), x=1.3, y=df['price'].median(), font_size=14, showarrow=False, font=dict(color="black", size=12)),
              dict(text=df['points'].median(), x=-0.2, y=df['points'].median(), font_size=14, showarrow=False, font=dict(color="black", size=12))])
    st.plotly_chart(fig, use_container_width=True)
